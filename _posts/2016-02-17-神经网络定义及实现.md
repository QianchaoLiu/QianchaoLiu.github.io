...
class MLP:
	def __init__(self, input, n_input, n_hiddens, n_output, activation=T.nnet.relu):
		...
		self.hidden_layers = []
		weight_matrix_size = zip(layers[:-1], layer[1:])
		data = input
		...
		for hidden in self.hidden_layers:
			self.param.append(hidden.params)
		self.param.append(self.output_layer.params)
...
# place holder
x = T.matrix('x').astyoe(theano.config.floatX)
y = T.ivector('y')
...
train_error = theano.function(inputs=[x, y, lr, reg], outputs = model.error_rate(y))
test_error = theano.function(inputs=[x, y, lr, reg], outputs = model.error_rate(y))

idx = numpy.arange(train_set_size)
for epoch in range(500):
	numpy.random.shuffle(inx)
